...
const ChatList = () => {
  ...
  useEffect(() => {
    ...
    if (lastNotif?.notification_type === 'message') {
      getConversations()
        .then((res) => setConversations(res.data))
        .catch(() => {});
    }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {conversations.map((conv) => (
              ...
              >
                ...
                {/* Right — badge + arrow together */}
                <div className="flex items-center gap-3 flex-shrink-0">
                  {conv.unread_count > 0 && (
                    <span className="w-6 h-6 bg-[#26187D] text-white text-xs rounded-full flex items-center justify-center font-bold">
                      {conv.unread_count}
                    </span>
                  )}
